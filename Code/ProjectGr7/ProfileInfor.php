  ...
  <body>
    <div class="container">
      ...
      <article>
        ...
        <ul class ="Hoten"> <?php $sql = "SELECT * FROM registration WHERE MaSoSV = '19522409'; ";
        ...
        <div class="MSSV" id="MSSVTABLE">
          <ul>
            MSSV: <?php $sql = "SELECT * FROM registration WHERE MaSoSV = '19522409'; ";
                    $result = mysqli_query($conn, $sql);
                    $resultCheck = mysqli_num_rows($result);
                    if($resultCheck > 0){
                      while($row = mysqli_fetch_assoc($result))
        ...
        <div class="NGAYSINH" id="NGAYSINHTABLE">
          <ul>
            Ngày sinh: <?php $sql = "SELECT * FROM registration WHERE MaSoSV = '19522409'; ";
                    $result = mysqli_query($conn, $sql);
                    $resultCheck = mysqli_num_rows($result);
                    if($resultCheck > 0){
                      while($row = mysqli_fetch_assoc($result))
        ...
        <ul>
            Lớp: <?php $sql = "SELECT * FROM registration WHERE MaSoSV = '19522409'; ";
                    $result = mysqli_query($conn, $sql);
                    $resultCheck = mysqli_num_rows($result);
                    if($resultCheck > 0){
                      while($row = mysqli_fetch_assoc($result))
      ...
      <div class="GIOITINH" id="GIOITINHSINHTABLE">
        <ul>
            Giới tính: <?php $sql = "SELECT * FROM registration WHERE MaSoSV = '19522409'; ";
                    $result = mysqli_query($conn, $sql);
                    $resultCheck = mysqli_num_rows($result);
                    if($resultCheck > 0){
                      while($row = mysqli_fetch_assoc($result))
      ...
      <form method="get" action="./Database/Logout.php">
        <button class="logout">
            <ul>
                LOG OUT
            </ul>
